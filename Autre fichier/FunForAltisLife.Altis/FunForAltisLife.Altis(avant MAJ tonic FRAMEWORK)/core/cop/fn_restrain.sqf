...
[] spawn
{
	...
	while {true} do
	{
		...
		if(!([west,getPos player,30] call life_fnc_nearUnits) && (player getVariable["restrained",FALSE]) && vehicle player == player) exitWith {
			...
			detach player;
			titleText["Du wurdest freigelassen, da die Zeit abgelaufen ist.","PLAIN"];
		};
	};
};

titleText[format["Tu t'es fais menotté par %1.",_cop getVariable["realname",name _cop]],"PLAIN"];
