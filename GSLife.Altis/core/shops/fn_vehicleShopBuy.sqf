...
_vehicleList = [life_veh_shop select 0] call life_fnc_vehicleListCfg; _basePrice = (_vehicleList select _vIndex) select 1;
 if(_mode) then {_basePrice = round(_basePrice * 5)};
...
if((life_veh_shop select 0) == "med_air_hs") then {
	...
	if(typeName _spawnPoints == typeName []) then {
		...
		{if(count(nearestObjects[(getMarkerPos _x),["Car","Ship","Air","McN_M1030","CIV_new_bike"],5]) == 0) exitWith {_spawnPoint = _x};} foreach _spawnPoints;
	} else {
		if(count(nearestObjects[(getMarkerPos _spawnPoints),["Car","Ship","Air","McN_M1030","CIV_new_bike"],5]) == 0) exitWith {_spawnPoint = _spawnPoints};
	};
};
